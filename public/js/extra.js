...
import {
  transform
} from 'markmap-lib/dist/transform.common'
import {
  markmap
} from 'markmap-lib/dist/view.common'
...
import {
  renderFretBoard
} from './lib/renderer/fretboard/fretboard'
import './lib/renderer/lightbox'
import {
  renderCSVPreview
} from './lib/renderer/csvpreview'
...
export function updateLastChangeUser() {
  if (window.lastchangeui) {
    if (window.lastchangeuser && window.lastchangeuserprofile) {
      ...
      if (window.lastchangeuserprofile.photo) {
        icon.attr('style', `background-image:url(${window.lastchangeuserprofile.photo})`)
      }
...
export function updateOwner() {
  if (ownerui) {
    if (window.owner && window.ownerprofile && window.owner !== window.lastchangeuser) {
      ...
      if (window.ownerprofile.photo && icon.attr('style') !== styleString) {
...
export function renderTitle(view) {
  ...
  if (title) {
    ...
    title = 'ShareNotes - Collaborative markdown notes'
  }
  return title
}
...
export function finishView(view) {
  ...
  for (let li of lis) {
    ...
    if (typeof editor !== 'undefined' && window.havePermission()) {
      disabled = ''
    }
    if (/^\s*\[[x ]]\s*/.test(html)) {
...
const emojijsPlugin = new Plugin(
  ...
  new RegExp(':(' + window.emojify.emojiNames.map((item) => {
